/*! Slug - v0.1.0 - 21/12/2021
* https://github.com/7clicksolucoes/slug
* Copyright (c) 2021 7clicksolucoes; Licensed MIT */
(function($) {

    $.fn.slug = function(source, options) {
        return this.each(function() {
            ...
            $source.on('keyup change',function() {
                ...
                if($target.is('input') || $target.is('textarea')) {
                    $target.val($.slug($source.val(), options));
                } else {
                    $target.text($.slug($source.val(), options));
                }
            });
        });
    };
    ...
    // Static method.
    $.slug = function(sourceString, options) {
        // Override default options with passed-in options.
        options = $.extend({}, $.slug.options, options);
